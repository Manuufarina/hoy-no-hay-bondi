    ...
    try {
      ...
      const geminiResponse = await fetchWithTimeout(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${geminiKey}`,
        {
          ...
          body: JSON.stringify({
            systemInstruction: {
              parts: [{ text: 'Sos un monitor de transporte público argentino. Tu tarea es buscar en la web información ACTUAL sobre paros de colectivos en Buenos Aires y responder SOLO con JSON puro, sin markdown ni texto extra. FUENTE PRINCIPAL OBLIGATORIA: parodebondis.com.ar — SIEMPRE consultá esta página PRIMERO, detalla línea por línea si hay paro o no. REGLAS CRÍTICAS: 1) PROHIBIDO GENERALIZAR — NUNCA digas "todas las líneas" o "varias líneas", listá CADA línea afectada individualmente como un objeto separado en lineas_afectadas con su número específico. 2) NO INVENTAR — si no encontrás paros concretos, respondé con hay_paros: false y lineas_afectadas vacío. 3) Indicá la fuente real de cada dato.' }],
            },
